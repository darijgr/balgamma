...
%TCIDATA{LastRevised=Wednesday, November 19, 2025 01:08:58}
...
{}
\theoremstyle{plainsl}
\ihead{An equality for balanced digraphs, version 2025-11-10}
\ohead{page \thepage}
\cfoot{}
\begin{document}
...
We will, in fact, prove a result more general than Theorem \ref{thm.balgamma},
replacing the acyclicity requirement in the definition of an $s$-convergence
by a precise set of cycles that we want our induced subdigraph $D\left\langle
B\right\rangle $ to have.
...
the set of all cycles of $D\left\langle A\right\rangle =D$.) Given a vertex
$s$ of $D$, we define an \emph{$s$-preconvergence} to be a (not necessarily
acyclic) subset $B$ of $A$ such that $s$ is a to-root of the subdigraph
$D\left\langle B\right\rangle $. For any $k\in\mathbb{N}$ and $s\in V$ and any
subset $Z$ of $\operatorname*{Cycs}A$, we let $\gamma_{k,Z}\left(  s\right)  $
denote the number of $s$-preconvergences $B$ of size $k$ (that is, with $k$
arcs) that satisfy $\operatorname*{Cycs}B=Z$. Then our general result is the following:
...
$Z=\varnothing$. Proving Theorem \ref{thm.gen} will therefore suffice in order
to prove Theorem \ref{thm.balgamma}.
...
Theorem \ref{thm.gen}, being a generalization of Theorem \ref{thm.balgamma},
inherits all these particular cases, but also leads to the following simple corollary:

\begin{corollary}
\label{cor.pre}Let $k\in\mathbb{N}$. Then, the number of all $s$%
-preconvergences of size $k$ (for a given $s\in V$) does not depend on $s$.
That is, for any $s,t\in V$, we have%
\begin{align*}
&  \left(  \text{number of all }s\text{-preconvergences of size }k\right) \\
&  =\left(  \text{number of all }t\text{-preconvergences of size }k\right)  .
\end{align*}

\end{corollary}

\begin{proof}
[Proof of Corollary \ref{cor.pre} using Theorem \ref{thm.gen}.]Let $s,t\in V$.
...
\[
\left(  \text{number of all }t\text{-preconvergences of size }k\right)
=\sum_{Z\subseteq\operatorname*{Cycs}A}\gamma_{k,Z}\left(  t\right)  .
\]
...
hand sides must also be equal. In other words,%
\begin{align*}
&  \left(  \text{number of all }s\text{-preconvergences of size }k\right) \\
&  =\left(  \text{number of all }t\text{-preconvergences of size }k\right)  .
\end{align*}
This proves Corollary \ref{cor.pre}.
\end{proof}

In the particular case when $D=G^{\operatorname*{bidir}}$ for some undirected
multigraph $G$, the claim of Corollary \ref{cor.pre} also follows from
\cite[Theorem 3.4]{Gioan07}, which also shows that both sides equal an
evaluation of the Tutte polynomial of $G$. We wonder if an appropriate
analogue of the Tutte polynomial exists for balanced digraphs that don't
necessarily have the form $G^{\operatorname*{bidir}}$.
...
in the proof of Lemma \ref{lem.new4} later on. (The impatient reader might
prefer to skip these fairly simple proofs and return to them as the lemmas are used.)
