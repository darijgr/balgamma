...
Lemma \ref{claim3}. Note that $s \in Q$, for else $B \in \Gamma_{k}(t)$.
...
On the other hand, suppose $v \in P$. We want to show that $v \in P'$, i.e. $v$ cannot $B'$-reach $s$. Since $s \in Q$, if $v$ could $B$-reach $s$, then there would be an arc in $A(P,Q)$ along this path from $v$ to $s$. But since $B' \cap A(P,Q) = \emptyset$, such a path would no longer exist in $B'$, yielding $v \in P'$. On the other hand, if $v$ could not $B$-reach $s$, then $v$ could not $B'$-reach $s$ either. To see this, in order for $v$ to $B'$-reach $s$, there would have to be an arc in $B' \cap A(Q,P)$ along this path, since $v$ could not $B$-reach $s$ beforehand. But since $v \in P$, this path would need additional arcs in $B' \cap A(P,Q)$, which is impossible. Hence, $v \in P'$. So, $P \subseteq P' \implies V \setminus P' = Q' \subseteq Q = V \setminus P$.
