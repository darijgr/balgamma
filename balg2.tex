...
%TCIDATA{LastRevised=Wednesday, July 16, 2025 10:13:30}
...
We have
\begin{align*}
\Psi_{P,Q}(\Phi_{P,Q}(B))  & = (\Phi_{P,Q}(B) \setminus A(Q,P)) \cup\phi
_{P,Q}^{-1}(\Phi_{P,Q}(B) \cap A(Q,P))\\
...
& = [B \cap A^{\complement}(P,Q) \cap A^{\complement}(Q,P)] \cup[\phi_{P,Q}(B)
\cap\underbrace{A(Q,P) \cap A^{\complement}(Q,P)}_{=\emptyset}]\\
...
\Gamma_{k}\left(  s,t\right)  \subseteq\mathcal{P}_{k}\left(  A\right)  $).
In  other words, $\Phi_{P,Q}\left(  B\right)  \in\mathcal{P}_{k}\left(
A\right)   $. Moreover, Lemma \ref{lem.realignment-acyc} \textbf{(b)} yields
that  $\Phi_{P,Q}\left(  B\right)  $ is acyclic (since $B\in\Gamma_{k}\left(
...
A\left(  P,Q\right)  =C\right\}  \right\vert \\
& =\left\vert X_{k-\left\vert C\right\vert }\right\vert
\end{align*}
for each $C\subseteq A\left(  P,Q\right)  $. This is done via bijection
$B\mapsto B\setminus A\left(  P,Q\right)  $ and inverse bijection $D\mapsto
D\cup A\left(  P,Q\right)  $. 
\end{proof}
