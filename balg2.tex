...
\href{mailto:mrbenny.jl@gmail.com}{\texttt{mrbenny.jl@gmail.com}}}}
...
\item[\textbf{(d)}] If $B \cap A(P,Q) = \emptyset$ instead, we then have $\Phi_{Q,P}\left( \Psi_{Q,P}\left( B \right) \right) = B$.
\end{enumerate}
\end{lemma}
...
\textbf{(c)} Let $A^{\complement}(P,Q) := (A(P,Q))^{\complement} = A \setminus A(P,Q)$ and $A^{\complement}(Q,P) := (A(Q,P))^{\complement} = A \setminus A(Q,P)$. 

We have \begin{align*}
    \Psi_{P,Q}(\Phi_{P,Q}(B)) &= (\Phi_{P,Q}(B) \setminus A(Q,P)) \cup \phi_{P,Q}^{-1}(\Phi_{P,Q}(B) \cap A(Q,P)) \\
    &= \Big([(B \setminus A(P,Q) ) \cup \phi_{P,Q}( B \cap A(P,Q))] \setminus A(Q,P)\Big) \\ 
    & \ \ \ \  \ \ \cup \phi^{-1}_{P,Q}\Big([(B \setminus A(P,Q)) \cup \phi_{P,Q}(B \cap A(P,Q))] \cap A(Q,P)\Big) \\
    &= [(B \setminus A(P,Q)) \setminus A(Q,P))] \cup [\phi_{P,Q}(B \cap A(P,Q)) \setminus A(Q,P)] \\
    & \ \ \ \ \ \ \cup \phi^{-1}_{P,Q}\Big([(B \setminus A(P,Q)) \cap A(Q,P)] \\
    & \ \ \ \ \ \ \cup [\phi_{P,Q}(B \cap A(P,Q)) \cap A(Q,P)]\Big) \\
    &= [\underbrace{B \setminus (A(P,Q) \cup A(Q,P))}_{=B \cap A^{\complement}(P,Q) \cap A^{\complement}(Q,P)}] \\
    & \ \ \ \ \ \ \cup [\phi_{P,Q}(B) \cap \underbrace{\phi_{P,Q}(A(P,Q))}_{=A(Q,P)} \cap A^{\complement}(Q,P)] \\
    & \ \ \ \ \ \ \cup[\phi^{-1}_{P,Q}(\underbrace{(B \setminus A(P,Q)) \cap A(Q,P)}_{=\emptyset \text{ since } B \cap A(Q,P) = \emptyset}] \\
    & \ \ \ \ \ \ \cup [\phi^{-1}_{P,Q}(\phi_{P,Q}(B \cap A(P,Q)) \cap A(Q,P))] \\
    &= [B \cap A^{\complement}(P,Q) \cap A^{\complement}(Q,P)] \cup [\phi_{P,Q}(B) \cap \underbrace{A(Q,P) \cap A^{\complement}(Q,P)}_{=\emptyset}] \\
    & \ \ \ \ \ \ \cup [\underbrace{\phi^{-1}_{P,Q}(\emptyset)}_{=\emptyset}] \cup [\underbrace{\phi^{-1}_{P,Q}(\phi_{P,Q}(B \cap A(P,Q)))}_{=B \cap A(P,Q)} \cap \underbrace{\phi^{-1}_{P,Q}(A(Q,P))}_{=A(P,Q)}] \\
...
\medskip
\textbf{(d)} It is clear that $\phi_{Q,P} = \phi_{P,Q}^{-1}$ and $\phi_{Q,P}^{-1} = \phi_{P,Q}$. Hence, $\Phi_{Q,P}(B) = (B \setminus A(Q,P)) \cup \phi_{Q,P}(B \cap A(Q,P)) = (B \setminus A(Q,P)) \cup \phi_{P,Q}^{-1}(B \cap A(Q,P)) = \Psi_{P,Q}(B)$ and similarly, $\Psi_{Q,P}(B) = \Phi_{P,Q}(B)$. So, under the assumption that $B \cap A(Q,P) = \emptyset$, a similar calculation to the previous part yields $\Phi_{Q,P}(\Psi_{Q,P}(B)) = B$.
\end{proof}
...
Let us first suppose $v \in Q$. Then $v$ cannot $B$-reach $t$. Since $B \in \Gamma_{k}(s,t)$, this means that $v$ can $B$-reach $s$. Suppose towards contradiction that $v \notin Q'$, i.e. that $v$ cannot $B'$-reach $s$. Then on the path in $B$ from $v$ to $s$, there must exist some vertex $w \in P$, for otherwise the entire path is contained in $Q$ and thus remains a path in $B'$. But if $w \in P$, then $v$ can $B$-reach $t$, a contradiction. Hence, $v \in Q'$, so $Q \subseteq Q'$.

On the other hand, suppose $v \in P$. We want to show that $v \in P'$, i.e. that $v$ cannot $B'$-reach $s$. First suppose $v$ can $B$-reach $s$. Since $s \in Q$, there must exist an arc in $B \cap A(P,Q)$ along this path from $v$ to $s$. But since $B' \cap A(P,Q) = \emptyset$, the path between $v$ and $s$ no longer exists in $B'$. Hence, $v \in P'$. On the other hand, suppose $v$ cannot $B$-reach $s$. Then $v$ cannot $B'$-reach $s$ either. To see this, in order for $v$ to $B'$-reach $s$, there would be an arc in $B' \cap A(Q,P)$ along this path from $v$ to $s$, since $v$ cannot $B$-reach $s$. But since $v \in P$, such a path would also need to contain additional arcs in $B' \cap A(P,Q)$, which is impossible since $B' \cap A(P,Q) = \emptyset$. Hence, $v \in P'$. In both cases, we see that $P \subseteq P' \implies V \setminus P' = Q' \subseteq Q = V \setminus P$.
...
\begin{proof}
\label{claim4}
   Let $B \in \Gamma_{k}(s,t) \setminus \Gamma_{k}(s)$. The proof of Lemma \ref{claim4} is the same as Lemma \ref{claim3}, but with $s,t,f,g,P,Q,P',Q',\Phi_{P,Q}$ and $\Psi_{P',Q'}$ renamed as $t,s,g,f,Q',P',Q,P,\Psi_{Q,P}$, and $\Phi_{Q',P'}$, and by envoking Lemma \ref{claim3} \textbf{(d)} instead of Lemma \ref{claim3} \textbf{(c)}.
\end{proof}
