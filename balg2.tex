...
%TCIDATA{LastRevised=Wednesday, July 23, 2025 10:38:42}
...
{}
\theoremstyle{plainsl}
\ihead{An equality for balanced digraphs DG, version \today}
\ohead{page \thepage}
\cfoot{}
\begin{document}

\title{An equality for balanced digraphs (DG version)}
...
that $\gamma_{k}\left(  s\right)  =\gamma_{k}\left(  t\right)  $. This is
clear if $s=t$, so we may assume without loss of generality that $s\neq t$.
...
$D\left\langle B\right\rangle $ contains a path $\mathbf{p}$ from $p$ to $s$.
Any vertex $v$ of this path $\mathbf{p}$ must itself belong to $S\left(
B\right)  $ (since it can $B$-reach $s$ by walking along $\mathbf{p}$ from $v$
to $s$), so it cannot belong to $Q$ (since $S\left(  B\right)  =P$ is disjoint
from $Q$). Therefore, no arc of this path $\mathbf{p}$ can belong to $A\left(
P,Q\right)  $ (since this would require its target to belong to $Q$). Thus,
all arcs of this path $\mathbf{p}$ belong to $B\setminus A\left(  P,Q\right)
=B\setminus C$. Hence, $\mathbf{p}$ is a path of $D\left\langle B\setminus
C\right\rangle $. Consequently, the vertex $p$ can $B\setminus C$-reach $s$
(via the path $\mathbf{p}$). In other words, $p\in S\left(  B\setminus
C\right)  $. Since we have proved this for each $p\in P$, we conclude that
$P\subseteq S\left(  B\setminus C\right)  $. Therefore, $S\left(  B\setminus
C\right)  =P$ (since $S\left(  B\setminus C\right)  \subseteq P$).
...
itself belong to $Q$ (since otherwise, $w$ would belong to $P=S\left(
...
the source of such an arc would lie in $P$ rather than $Q$). Thus, all arcs of
$\mathbf{q}$ belong to $B\setminus A\left(  P,Q\right)  =B\setminus C$. Hence,
$\mathbf{q}$ is a path of $D\left\langle B\setminus C\right\rangle $. This
shows that $q$ can $B\setminus C$-reach $t$ (via the path $\mathbf{q}$). In
other words, $q\in T\left(  B\setminus C\right)  $. Since we have proved this
for each $q\in Q$, we thus conclude that $Q\subseteq T\left(  B\setminus
C\right)  $.
