...
%TCIDATA{LastRevised=Wednesday, July 23, 2025 19:20:23}
...
$s$, meaning $q\in S\left(  E\right)  $. Thus, $q\in S\left(
...
\begin{proof}
[Proof of Claim 1.]Let $B\in Y_{k}$. Then $B\in U_{k}$, $S\left(  B\right)
...
from $q$ until $w$ and then following the path $\mathbf{w}$ from $w$ to $s$, contradicting the fact that $q$ cannot $B$-reach $s$). Therefore,
...
C\in U_{k}$, $S\left(  E\cup C\right)  =P$, and $\left(  E\cup C\right)
